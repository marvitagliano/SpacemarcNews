...
$action = (isset($_POST['btndel'])) ? 'utenti.php' : 'profilo_admin.php?user_id=' . $q_user_id;
...
$data_pubb = ($q_riga['TotaleNews'] == 0) ? NULL : ' - ' . $lang['ultima'] . ' ' . $ultima_inserita . ' - <a href="gestione_news.php?user_id=' . $q_riga['user_id'] . '"><img src="' . $img_path . '/news.png" border="0" alt="" /> ' . $lang['leggi'] . '</a> <a href="javascript:;" onclick="window.open(\'sposta_news.php?user_id=' . $q_riga['user_id'] . '\', \'\', \'width=400, height=220, resizable=1, scrollbars=1, location=1, status=1\');" title="[Popup]"><img src="' . $img_path . '/sposta.png" border="0" alt="" /> ' . $lang['sposta_news'] . '</a>';
...
$autorizza_news = ($q_riga['autorizza_news'] == 1) ? '<input type="checkbox" name="user_autorizza_news" id="user_autorizza_news" checked="checked" /><label for="user_autorizza_news">' . $lang['autorizza_news'] . '</label>' : '<input type="checkbox" name="user_autorizza_news" id="user_autorizza_news" /><label for="user_autorizza_news">' . $lang['autorizza_news'] . '</label>';
...
$livello = $lang['livello'] . ': <b>' . $q_riga['nome_livello'] . '</b>';
$cbdelfile_dis = ($files == 0) ? 'disabled="disabled"' : NULL;
$user_attivo = ($q_riga['attivo'] == 1) ? '- Status: ' . $lang['attivo'] . '' : '- Status: <span style="color: rgb(255, 0, 0);">' . $lang['status_disattivo'] . '</span>';
$user_attivo2 = ($q_riga['attivo'] == 1) ? '<input type="checkbox" name="user_disattivo" id="user_disattivo" onclick="if (this.checked) { alert(\'' . $lang['attenzione_utente_disattivato'] . '\'); }" /><label for="user_disattivo">' . $lang['disattiva_account'] . '</label>' : '<input type="checkbox" name="user_attivo" id="user_attivo" /><label for="user_attivo">' . $lang['attiva_account'] . '</label>';
$user_autorizza_news = ($q_riga['autorizza_news'] == 1) ? '' : '<span style="color: rgb(255, 0, 0);">' . $lang['user_autorizza_news'] . '</span>';
$cb_cookie = ($q_riga['cookie'] == 1) ? '<input type="checkbox" name="cookie" id="cookie" checked="checked" /><label for="cookie" class="help" style="border-bottom: 1px dotted #000;" title="' . $lang['ricorda_title'] . '">' . $lang['ricorda'] . '</label>' : '<input type="checkbox" name="cookie" id="cookie" /><label for="cookie" class="help" style="border-bottom: 1px dotted #000;" title="' . $lang['ricorda_title'] . '">' . $lang['ricorda'] . '</label>';

//posso cancellare solo un utente che non ha scritto news

if ($q_riga['TotaleNews'] == 0) {
    $btn_cancella_utente = '<input type="submit" name="btndel" id="btndel" value="' . $lang['cancella_utente'] . '" style="font-weight: bold;" onclick="return confirmSubmit()" />';
    $btn_cancella_news = NULL;
}
else {
    $btn_cancella_utente = NULL;
    $btn_cancella_news = '<input type="checkbox" name="cbdelnews" id="cbdelnews" title="' . $lang['cancella_news'] . ' " /><label for="cbdelnews">&#129152;</label> <input type="submit" name="btndelnews" id="btndelnews" value="' . $lang['cancella_news'] . ' &#9888;" style="font-weight: bold;" onclick="alert(\'' . $lang['attenzione_news'] . '\'); return confirmSubmit()" />';
}

//se l'utente visualizzato non è l'utente loggato

if ($q_user_id != $_SESSION['user_id']) {
    $link_invia_email = ' - <a href="javascript:;" onclick="window.open(\'invia_email_utenti.php?user_id=' . $q_user_id . '\', \'\', \'width=420, height=435, resizable=1, scrollbars=1, location=1, status=1\');" title="[Popup]"><img src="' . $img_path . '/mail.png" alt="Email" border="0" /> ' . $lang['invia_email'] . '</a>';
    $cancella_utente = '<tr><td bgcolor="#DEE3E7" align="left"><b class="text">' . $lang['canc_disatt'] . '</b></td>
														<td bgcolor="#EEEEEE" align="left" class="text">' . $btn_cancella_utente . $btn_cancella_news . $user_attivo2 . ' <input type="checkbox" name="cbdelfile" id="cbdelfile" onclick="if (this.checked) { alert(\'' . $lang['attenzione_file'] . '\'); }" ' . $cbdelfile_dis . ' /><label for="cbdelfile"><span style="color: rgb(255, 0, 0);">' . $lang['delete'] . ' ' . $files . ' ' . $lang['file'] . '</span></label> ' . $link_files . '</td></tr>';
}
else {
    $link_invia_email = NULL;
    $cancella_utente = '<tr><td bgcolor="#DEE3E7" align="left"><b class="text">' . $lang['canc_disatt'] . '</b></td>
		                        <td bgcolor="#EEEEEE" align="left" class="text"> ' . $btn_cancella_news . ' <input type="checkbox" name="cbdelfile" id="cbdelfile" onclick="if (this.checked) { alert(\'' . $lang['attenzione_file'] . '\'); }" ' . $cbdelfile_dis . ' /><label for="cbdelfile"><span style="color: rgb(255, 0, 0);">' . $lang['delete'] . ' ' . $files . ' ' . $lang['file'] . '</span></label> ' . $link_files . '</td></tr>';
}
...
$user_permessi_view = '<tr><td bgcolor="#DEE3E7" align="left"><b class="text">' . $lang['permessi'] . '</b></td>
					<td bgcolor="#EEEEEE" align="left" class="text">
		                           <input type="radio" id="rb_upload" name="rbp" value="upload"' . $permessi_selected1 . ' /><label for="rb_upload">' . $lang['file_upload'] . '</label>
		                           <input type="radio" id="rb_letture" name="rbp" value="letture"' . $permessi_selected2 . ' /><label for="rb_letture">' . $lang['modifica_letture'] . '</label>
		                           <input type="radio" id="rb_tutto" name="rbp" value="tutto"' . $permessi_selected3 . ' /><label for="rb_tutto">' . $lang['tutti_permessi'] . '</label>
		                           <input type="radio" id="rb_nessuno" name="rbp" value="nessuno"' . $permessi_selected4 . ' /><label for="rb_nessuno">' . $lang['nessun_permesso'] . '</label> ' . $autorizza_news . '</td></tr>';
...
if (isset($_POST['submit'])) {
	...
	if ($user_password_ok == 1 && $user_nome_cognome_ok == 1 && $user_email_ok == 1 && $email_ok == 1 && $user_data_nascita_ok == 1 && $facebook_ok == 1 && $twitter_ok == 1 && $instagram_ok == 1) {
        ...
        if (isset($_POST['btndel']) || isset($_POST['user_attivo']) || isset($_POST['user_disattivo'])) {
            operazioni_utente();
        }
        else {
...
<textarea name="hobby" cols="36" rows="3"><?php echo $hobby_value; ?></textarea></td>                              
        ...
        <tr>                                       
          ...
          <td bgcolor="#EEEEEE" align="left" class="text">                                             
            ...
            <input type="radio" id="rb_instagram" name="rb" value="instagram" <?php echo $link_selected7; ?> /><label for="rb_instagram"><?php echo $lang['mostraig']; ?></label></td>
        </tr>                         
<?php echo $user_permessi_view; echo $cancella_utente; echo $delete_news_msg; ?>                                 
        <tr>                                       
